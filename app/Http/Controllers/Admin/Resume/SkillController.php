...
class SkillController extends Controller
{
    public function index(Request $request)
    {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    $btn = '<div class="d-flex justify-content-start flex-shrink-0">
                        ...
                        <a href="javascript:;" onclick="deleteSkill(' . $row->id . ')" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
        ...
        return view('admin.resume.skill.skill_index');
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'name' => ['required', $request->skill_id == null ? 'unique:skills' : '' ],
            'type' => 'required',
        ]);
        ...
        if ($validator->fails()) {
            ...
            if (isset($request->skill_id) &&  $skill = Skill::find($request->skill_id)) { //update
                //dd($request->skill_id);
