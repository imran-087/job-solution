...
class PassageController extends Controller
{
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    $btn = '<div class="d-flex justify-content-start flex-shrink-0">
                        ...
                        <a href="javascript:;" onclick="deletePassage(' . $row->id . ')" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (isset($request->passage_id) &&  $passage = Passage::find($request->passage_id)) { //update
                ...
                if ($passage->update()) {
                    return response()->json([
                        'success' => true,
                        'message' => __('Passage updated successfully!')
                    ], 200);
                } else {
                    return response()->json([
                        'error' => true,
